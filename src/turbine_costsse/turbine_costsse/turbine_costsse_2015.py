...
class Turbine_CostsSE_2015(Assembly):
	...
	lss_mass_cost_coeff = Float(12.60, iotype='in', units='$/kg', desc='low speed shaft mass-cost coefficient [$/kg]')	#JMF these have to be listed here too, correct?
    main_bearing_mass = Float(iotype='in', units='kg', desc='component mass [kg]') #mass input
    #second_bearing_mass = Float(iotype='in', units='kg', desc='component mass [kg]') #JMF are we still going to have this?
	bearings_mass_cost_coeff = Float(12.70, iotype='in', units='$/kg', desc='main bearings mass-cost coefficient [$/kg]')	
	#JMF
    ...
    #JMFadvanced_blade = Bool(True, iotype='in', desc='advanced (True) or traditional (False) blade design')
    #JMFdrivetrain_design = Enum('geared', ('geared', 'single_stage', 'multi_drive', 'pm_direct_drive'), iotype='in')
    #JMFcrane = Bool(iotype='in', desc='flag for presence of onboard crane')
    #JMFoffshore = Bool(iotype='in', desc='flag for offshore site')
    ...
    def configure(self):
        ...
        self.replace('towerCC', Tower_CostsSE()) #JMF
        self.replace('tcc', TurbineCostAdder()) #JMF

        # connect inputs
		#JMF add connect statements for all mass_cost_coeffs
...
class TurbineCostAdder(Component): #JMF to be updated
